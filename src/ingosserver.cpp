...
String displayVersion = "";  //wird in main.cpp setup() gesetzt
u16_t Reconnect_Zaehler = 0;
int Aufruf_Zaehler = 0;
int switch_color=0;
String request;   //Anfrage des HTML-Clients (also vom Browser)
...
//********************************************************************************************************************
//in main->setup() aufrufen!!
void initIngosServer(){
     
  server.begin(); //Init Web-Server
} 
//********************************************************************************************************************
 //der oben definierte client word +ber server.accept ermittelt und wird zurückgegeben
 WiFiClient serverAccept(){
   client = server.accept();
   return client;
};

//********************************************************************************************************************
//Beendet eine bestehende Verbindung des client, der mit serverAccept verbunden wurde
void clientStop(){
  client.stop();
} 
...
bool clientReceiveGETRequest(){
  ...
  isGETRequest = contains("GET", request.c_str());

  #ifdef DEB_HTML
    Serial.printf("request=[%s]\n", request.c_str());                    // gelesenes auf Serial ausgeben
...
//
//params:
//tm_str ist der Zeitstempel, der in der grauen Zeile am Ende der HTML-Seite ausgegeben wird
//current_index ist der aktuelle Index, die im Balkendiagramm hervorgehoben werden soll
...
//******************** Preise-Balkendiagramm als SVG ***********************
  ...
  clientPrintSVGBarChart(current_index);
  clientPrintSVGFooter();

//*********************** graue Statuszeilee *******************************
  client.printf("<br><p style=\"font-size:11px;color:gray\"> Version: %s, RSSI: %d, %s, recon=%d, count=%d\r\n",
                                 displayVersion.c_str(), wifi_station_get_rssi(), tm_str, Reconnect_Zaehler, Aufruf_Zaehler++);
//*********************** HTML Ende **************************
  client.println("</body>");
  client.println("</html>");
...
//************************
//Balkengrafik als SVG an client senden
//client muss available()=true sein
//current_index ist der Index der hervorgehoben werden soll
